...
def strategy_switcher(msg, isGroupChat=False):
    ...
    if isGroupChat and msg.isAt:
        text = bytes.decode(msg.text.split(u"\u2005", 2)[-1].encode("utf-8"))
    else:
        text = bytes.decode(msg.text.encode("utf-8"))
    #个人注册函数触发检测
...
def _chat_log(msg, isGroupChat=False):
    text = bytes.decode(msg.get('Text', u"").encode("utf-8"))
    #发送方
    fromUser = itchat.search_friends(userName=msg["FromUserName"])
    if fromUser is None:
        fromUserName = "Stranger"
    else:
        fromUserName = bytes.decode(fromUser["NickName"].encode("utf8"))
    #接收方
    if isGroupChat:
    ...
    if toUser is None:
        ...
        toUserName = bytes.decode(toUser["NickName"].encode("utf8"))
    log.notice("{} -> {} : {}".format(fromUserName, toUserName, bytes.decode(msg["Content"].encode("utf8"))))
